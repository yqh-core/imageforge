...
// 默认界面语言取自 brand.config.json 的 locale，不再硬编码。
// 只取主语言子标签（zh-CN → zh），因为语言包是按 src/js/languages/<code>.json
// 命名的，带地区的标签会找不到文件。可用值见该目录：ar de el es fr it ja ko lt pt ru tr uk zh。
// 用户可在 Help → Language 里覆盖（结果存 cookie），也支持 URL 传 ?lang=xx 强制指定，
// 覆盖逻辑见 core/base-gui.js 的 load_translations()。
config.LANG = String(brand.locale || 'en').split('-')[0].toLowerCase();
config.WIDTH = null;
config.HEIGHT = null;
