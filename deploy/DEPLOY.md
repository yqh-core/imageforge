...
`verify` 回答的是"页面能打开吗、品牌对吗、安全头在吗"。它证明不了**编辑器真的能用** ——
一个 bundle 加载成功但工具栏全哑的页面，在 verify 眼里是完全健康的。
`probe`（`tools/verify/features.js`）补的就是这一段，16 项，真的去操作界面：

- **绘制**：新建画布 → 选画笔 → 用 CDP 派发真实鼠标事件拖一笔 → 数画布上
  非透明像素（必须 > 0，不是"函数被调用了"）
- **撤销 / 重做往返**：撤销要把笔画去掉，重做要把它原样带回来 ——
  只测单向的话，"撤了就拉不回来"照样判绿
- **图层**：点插入图层，面板行数必须**恰好** +1
- **工具**：41 个工具逐个激活，任何一个抛异常都算失败，并关掉途中弹出的对话框
- **导出**：真的走完 File → Export → 确认对话框，然后把交给下载机制的 blob
  取回来**按内容验**：PNG 魔数 + IHDR 里的宽高（1024×768），
  再确认 `<a download>` 上的文件名是 `*.png`
- **语言**：Tools → Language → 简体中文 之后，菜单栏首项要变成"文件"

两个实测踩到的坑，改这个脚本时容易再犯：

- file-saver 是用 `a.dispatchEvent(new MouseEvent('click'))` 触发下载的，**不是** `a.click()`。
  只钩 `click` 会一个文件名都抓不到，看起来像导出坏了，其实导出完全正常。
- 点完 Undo 后 Edit 下拉会被关掉，Redo 那个链接此时**不在 DOM 里**。
  直接查它会得到"no redo entry / 0 px"，读起来像撤销栈坏了。

`probe` 会操作界面、改变状态，所以耗时比 `verify` 长，也**必须放在 `verify` 之后单独跑**
（或至少别指望它跑完页面还是干净的）。语言那一段放在最后，跑完会切回英文。
